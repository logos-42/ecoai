...
const PolicySimulator: React.FC<PolicySimulatorProps> = ({ className }) => {
          ...
          <div className="space-y-4">
            <div className="space-y-2">
              <div className="flex items-center justify-between">
                ...
                <span className="text-sm text-muted-foreground">{tariffRate}%</span>
              </div>
              <Slider
                id="tariff-rate"
                value={[tariffRate]}
  ...
  const ImpactCard = ({ title, items }: { title: string; items: string[] }) => (
    <div className="rounded-lg bg-white/50 p-4 border border-border">
  ...
  return (
    <div className={cn("flex flex-col h-full rounded-2xl overflow-hidden glass-card", className)}>
      <div className="px-6 py-4 border-b">
        <h2 className="text-xl font-semibold">政策模拟器</h2>
        <p className="text-sm text-muted-foreground">模拟不同经济政策的影响</p>
      </div>
      ...
      <div className="p-6 space-y-6 flex-1">
        ...
        <AnimatedTransition show={!!simulationResult} variant="slide-up" className="space-y-6">
          {simulationResult && (
            <div className="space-y-6">
              <h3 className="text-lg font-semibold">{simulationResult.name}</h3>
              <p className="text-muted-foreground">{simulationResult.description}</p>
              ...
              <div className="mt-6">
                <h4 className="font-semibold mb-4">历史案例参考</h4>
                <div className="space-y-3">
                  {simulationResult.historicalExamples.map((example, index) => (
                    <div key={index} className="flex items-start gap-3 p-3 rounded-lg bg-white/50 border border-border">
                      <div className="flex-shrink-0 w-12 h-12 rounded-full bg-econoBlue-light flex items-center justify-center text-econoBlue font-semibold">
                        {example.year}
                      </div>
                      <div>
                        <div className="font-medium">{example.country}</div>
                        <div className="text-sm text-muted-foreground">{example.outcome}</div>
                      </div>
                    </div>
                  ))}
                </div>
        ...
        {!simulationResult && !isLoading && (
          <div className="flex-1 flex flex-col items-center justify-center text-center p-8">
            ...
            <h3 className="text-lg font-medium mb-2">调整参数并运行模拟</h3>
